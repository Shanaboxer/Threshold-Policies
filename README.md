...
Then check it's working: click the CoolHeaded icon → **Check it's working**.
...
queries everywhere, not just on blocked sites. CoolHeaded has no opinion about
...
```
bash: ./install-coolheaded.sh: Permission denied
sudo: ./install-coolheaded.sh: command not found
```
...
When you set CoolHeaded up, you were asked two things while you were calm.
...
> **Tip:** at the end of setup, CoolHeaded offers to save a file called
> `coolheaded-how-to-undo.txt`. That file has *your own* answers written into
...
1. Open CoolHeaded's settings in your browser
...
Then: browser menu → Extensions → CoolHeaded → Remove.
