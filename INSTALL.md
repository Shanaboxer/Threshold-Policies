...
1. Unzip Threshold somewhere permanent — **Documents, not Downloads.** The
...
5. Select the **`threshold` folder itself** — the one containing
...
Click the Threshold icon in the toolbar, then **Check it's working**. You want:
...
This creates `threshold/firefox/build/`.

### 2. Load it

1. Go to `about:debugging#/runtime/this-firefox`
2. Click **Load Temporary Add-on…**
3. Select **`threshold/firefox/build/manifest.json`**
...
**Any browser you skip is an open door.** If Threshold is in Chrome but you
...
Chromium browsers: `chrome://extensions` → the reload icon on the Threshold
