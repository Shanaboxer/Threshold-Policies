...
There is no account, no sign-in, and no server belonging to CoolHeaded. We have
not built one, so there is nowhere for your information to be sent even if we
wanted it.

## What is stored, and where

Everything CoolHeaded knows about you lives in your own browser's local storage,
on your own machine:
...
CoolHeaded does **not** keep a record of:
...
Once a day, CoolHeaded downloads a public blocklist file from GitHub — the
...
## If CoolHeaded is ever paid for
...
CoolHeaded would still hold no account linked to your use of it.

## Removing it

Uninstalling CoolHeaded deletes its local storage along with everything above.
